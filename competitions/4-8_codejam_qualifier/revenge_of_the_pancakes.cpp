/*
  ...
  of pancakes to serve to a customer. Like any good pancake server, you have X-ray
  ...
  (possibly all of them) from the top of the stack, flip that entire group over, and 
  then put the group back down on top of any pancakes that you did not lift up.
  When flipping a group of pancakes, you flip the entire group in one motion; 
  ...
  and remain unchanged). The new state of the stack would then be -++-. 
  ...
  You will not serve the customer until every pancake is happy side up, but 
  you don't want the pancakes to get cold, so you have to act fast! What is the
  smallest number of times you will need to execute the maneuver to get all the pancakes
  ...
  case number (starting from 1) and y is the minimum number of times you will need to 
  ...
  xthen flip the top pancake to get --++, then finally flip the top two pancakes to get ++++.
*/
...
/*
  my solution is to break the problem into smaller subproblems
  first subproblem is largest: how many flips to get original string to all +?
  if there's a minus, find out how many flips to get piece ending with that minus to all -s? (then flip all those to +)
  this continues recursively

  example:
  --+-, how many flips to get all +?
  --+-, how many flips to get all -?
  --+, how many flips to get all +?
  --, how many flips to get all -? 0
  0 + 1 + 1 + 1 = 3 total flips
 */
